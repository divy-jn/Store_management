    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for i in range(0, len(events), batch_size):
            ...
            try:
                ...
                data = response.json()
                logger.info(
                    f"Batch {i//batch_size + 1}: Accepted {data['accepted']}, Rejected {data['rejected']}, Duplicates {data['duplicates']}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to ingest batch {i//batch_size + 1} after retries: {e}"
                )


async def main():
    parser = argparse.ArgumentParser(
        description="Replay JSONL events to Store Intelligence API"
    )
    parser.add_argument(
        "--events-dir", required=True, help="Directory containing JSONL event files"
    )
    parser.add_argument(
        "--api-url",
        default="http://localhost:8000",
        help="URL of the Store Intelligence API",
    )
